...
namespace Pathinox
{
    public class GameHub : Hub
    {
        ...
        public async Task ConnectGame(string gameCode, object playerData)
        {
            ...
            var playerUniqueId= jsonElement.GetProperty("uniqueid").GetString() ?? "Unknown";
            ...
            lock (_lock)
            {
                ...
                {
                    ConnectGameCreateNew(gameCode, playerName, playerUniqueId, playerColor, playerColorAlt);
                    bCreategame = true;
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        var player = gameState.Players.FirstOrDefault(p => p.UniqueId == playerUniqueId);
                        if (player != null)
                        {
                            bReconnect = true;
                        ...
                        else
                        {
                            if (_gameRoomsState.GameRooms[gameCode].Players.Count < 2)
                            {
                                ...
                                {
                                    bError = true;
                                    errorMsg = $"Player '{playerName}' already in this game. Amend your name.";
                                }
                                else
                                {
                                    bJoin = true;
                                    ConnectGameAddPlayer(gameCode, playerName, playerUniqueId, playerColor, playerColorAlt);
                                }
                            }
                            else
                            {
        ...
        private void ConnectGameCreateNew(string gameCode, string playerName, string playerUniqueId, string playerColor, string playerColorAlt)
        {
        ...
        private bool ConnectGameAddPlayer(string gameCode, string playerName, string playerUniqueId, string playerColor, string playerColorAlt)
        {
            var gameState = _gameRoomsState.GameRooms[gameCode];
